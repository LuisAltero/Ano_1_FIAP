-- EXEMPLO - RELACIONAMENTO NAO OBRIGATÓRIO - SCRIPT DDL
----------------------------------------------------------
...
-- RECUPERAMOS ALUNOS MATRICULADOS ATRAVES DO CONVENIO (LINHAS COM TODOS OS CAMPOS PREENCHIDOS)
-- RECUPERAMOS ALUNOS MATRICULADOS QUE NÃO ESTÃO ASSOCIADOS A NENHUMA EMPRESA CONVENIADA (NULL EMPRESA CONV.)
-- RECUPERAMOS EMPRESAS CONVENIADAS QUE NÃO POSSUEM NENHUM ALUNO MATRICULADO ATRAVES CONVENIO (NULL MATRICULA/ALUNO)
-- FULL OUTER JOIN
SELECT EC.CD_EMPRESA_CONV, EC.CD_EMPRESA_CONV, M.NR_MATRICULA_ALUNO, M.CD_ALUNO
  FROM T_SCA_EMPRESA_CONV EC FULL OUTER JOIN T_SCA_MATRICULA M
  ON (EC.CD_EMPRESA_CONV = M.CD_EMPRESA_CONV);

-- FULL OUTER JOIN (TRAZER APENAS QUEM NÃO POSSUI ASSOCIAÇÃO DOS DOIS LADOS/TABELAS)
-- RECUPERAMOS ALUNOS MATRICULADOS QUE NÃO ESTÃO ASSOCIADOS A NENHUMA EMPRESA CONVENIADA (NULL EMPRESA CONV.)
-- RECUPERAMOS EMPRESAS CONVENIADAS QUE NÃO POSSUEM NENHUM ALUNO MATRICULADO ATRAVES CONVENIO (NULL MATRICULA/ALUNO)
SELECT EC.CD_EMPRESA_CONV, EC.CD_EMPRESA_CONV, M.NR_MATRICULA_ALUNO, M.CD_ALUNO
  FROM T_SCA_EMPRESA_CONV EC FULL OUTER JOIN T_SCA_MATRICULA M
